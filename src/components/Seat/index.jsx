...
export const Seat = ( {number, isOccupied, isSelected, onSelect} ) => {
  let obsazeno = "seat" + (isOccupied ? ' seat--occupied':'');
  let vybrano =  "seat" + (isSelected ? ' seat--selected':'');
  ...
  const handleSelected = () => {
    ...
    return(
      <>
        <svg onClick={() => (!isOccupied ? handleSelected() :'')} 
             className={isSelected? vybrano : obsazeno} 
             viewBox="0 0 100 100" role="button">
